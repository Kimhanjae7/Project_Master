...
app.get('/get_popular_projects', (req, res) => {
  ...
  // 조회수에 따라 프로젝트 정렬
  let sql = `
  SELECT p.category, p.project_seq, p.introduce_title, p.teck_stack, p.position, p.deadline, m.nickname, m.profile_pic, p.view_count
  FROM project_info p
  JOIN member m ON p.user_id = m.user_id
...
app.get('/get_recent_projects', (req, res) => {
  ...
  let sql = `
  SELECT p.category, p.project_seq, p.introduce_title, p.teck_stack, p.position, p.deadline, m.nickname, m.profile_pic
  FROM project_info p
  JOIN member m ON p.user_id = m.user_id
...
app.get('/filter_projects', (req, res) => {
  ...
  let sql = `SELECT p.category, p.project_seq, p.introduce_title, p.teck_stack, p.position, p.deadline, m.nickname, m.profile_pic, m.user_id, p.view_count
             FROM project_info p
             JOIN member m ON p.user_id = m.user_id
             WHERE p.status = 'active'`;
...
app.post('/joinProc', (req, res) => {
  ...
  //---수정
  const imagePath = 'C:\\Users\\재희\\Desktop\\project\\public\\images\\default_profile.png';
  ...
  connection.query(checkSql, [user_id, nickname], function(err, result) {
    ...
    if (result.length > 0) {
      ...
      if (result.some(row => row.user_id === user_id)) {
          errorMessage += '아이디';
      } else if (result.some(row => row.nickname === nickname)) {
          errorMessage += '닉네임';
      }
      errorMessage += '입니다.';
      ...
      connection.query(insertSql, values, function(err, result) {
        ...
        var insertGradeSql = `INSERT INTO grade (user_id) VALUES (?)`;
        connection.query(insertGradeSql, [user_id], function(gradeErr, gradeResult) {
...
app.get('/project/:project_seq', (req, res) => {
  ...
  const selectSql = `
  SELECT pi.*, m.nickname AS author_nickname, TO_BASE64(m.profile_pic) AS author_profile_pic_base64
  FROM project_info pi
  LEFT JOIN member m ON pi.user_id = m.user_id
  WHERE project_seq = ?`;
  ...
  connection.query(selectSql, [project_seq], (err, result) => {
      ...
      if (result.length > 0) {
          ...
          if (project.author_profile_pic_base64) {
            project.author_profile_pic_base64 = `data:image/png;base64,${project.author_profile_pic_base64}`;
        }

          if (isAuthor) {
            const participantsSql = `
            SELECT m.nickname, TO_BASE64(m.profile_pic) AS profile_pic_base64, p.user_id, p.position, p.status 
            ...
            WHERE project_seq = ? AND check_writer = 'Participant'`;
              ...
              connection.query(participantsSql, [project_seq], (err, participantsResult) => {
                  ...
                  res.render('post_view_write', {
                      projects: project,
                      isAuthor: true,
                      participants: participantsResult,
                      authorNickname: project.author_nickname,
                      authorProfilePic: project.author_profile_pic_base64
                  });
              });
          } else {
...
app.get('/see_project', (req, res) => {
  ...
  const projectSql = `SELECT p.introduce_title, p.category, p.deadline, p.position, p.teck_stack, p.project_seq, p.view_count, m.nickname
                      FROM project_info p
                      INNER JOIN member m
  ...
  connection.query(projectSql, [user_id], (err, projectResults) => {
    ...
    connection.query(memberSql, [user_id], (err, memberResults) => {
      ...
      const participantProjectsSql = `
        SELECT pi.introduce_title, pi.category, pi.deadline, pi.position, pi.teck_stack, pi.project_seq, m.nickname, m.profile_pic, pi.view_count
        FROM project_participants p
      ...
      connection.query(participantProjectsSql, [user_id], (err, participantResults) => {
        ...
        connection.query(appliedProjectsSql, [user_id], (err, appliedProjects) => {
          ...
          const endProjectsSql = `
          SELECT pi.introduce_title, pi.category, pi.deadline, pi.position, pi.teck_stack, pi.project_seq, m.nickname, m.profile_pic, pi.view_count
          FROM project_participants p
          INNER JOIN project_info pi ON p.project_seq = pi.project_seq
          INNER JOIN member m ON pi.user_id = m.user_id
          WHERE p.user_id = ? AND pi.status = "complete"
        `;
          
        connection.query(endProjectsSql, [user_id], (err, endProjects) => {
          if (err) {
            console.error('Database query error:', err);
            res.send("<script> alert('지원한 프로젝트 정보를 가져오는 중 오류가 발생했습니다.'); location.href='/';</script>");
            return;
...
app.post('/end_project', (req, res) => {
  ...
  const updateSql = 'UPDATE project_info SET status = ? WHERE project_seq = ?';
  connection.query(updateSql, ['complete', project_seq], (err, result) => {
      if (err) throw err;
      res.render('popup', {
...
app.get('/admin/grades', adminAuth, (req, res) => {
  const searchQuery = req.query.search || '';
  ...
  connection.query(sql, [`%${searchQuery}%`], (err, results) => {
      if (err) throw err;
      res.render('admin_grades', { grades: results, searchQuery }); // 'admin_grades.ejs' 템플릿에 데이터 전달
  });
});

// 평점 생성
app.post('/admin/grades/create', adminAuth, (req, res) => {
  const { user_id, Front_end_count, Front_end_rating_count, Front_end_total_rating, Front_end_average_rating, Back_end_count, Back_end_rating_count, Back_end_total_rating, Back_end_average_rating, Designer_count, Designer_rating_count, Designer_total_rating, Designer_average_rating, IOS_count, IOS_rating_count, IOS_total_rating, IOS_average_rating, Android_count, Android_rating_count, Android_total_rating, Android_average_rating, DevOps_count, DevOps_rating_count, DevOps_total_rating, DevOps_average_rating, PM_count, PM_rating_count, PM_total_rating, PM_average_rating, Planner_count, Planner_rating_count, Planner_total_rating, Planner_average_rating, Marketer_count, Marketer_rating_count, Marketer_total_rating, Marketer_average_rating } = req.body;

  const sql = `INSERT INTO grade (user_id, Front_end_count, Front_end_rating_count, Front_end_total_rating, Front_end_average_rating, Back_end_count, Back_end_rating_count, Back_end_total_rating, Back_end_average_rating, Designer_count, Designer_rating_count, Designer_total_rating, Designer_average_rating, IOS_count, IOS_rating_count, IOS_total_rating, IOS_average_rating, Android_count, Android_rating_count, Android_total_rating, Android_average_rating, DevOps_count, DevOps_rating_count, DevOps_total_rating, DevOps_average_rating, PM_count, PM_rating_count, PM_total_rating, PM_average_rating, Planner_count, Planner_rating_count, Planner_total_rating, Planner_average_rating, Marketer_count, Marketer_rating_count, Marketer_total_rating, Marketer_average_rating) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;

  const params = [user_id, Front_end_count, Front_end_rating_count, Front_end_total_rating, Front_end_average_rating, Back_end_count, Back_end_rating_count, Back_end_total_rating, Back_end_average_rating, Designer_count, Designer_rating_count, Designer_total_rating, Designer_average_rating, IOS_count, IOS_rating_count, IOS_total_rating, IOS_average_rating, Android_count, Android_rating_count, Android_total_rating, Android_average_rating, DevOps_count, DevOps_rating_count, DevOps_total_rating, DevOps_average_rating, PM_count, PM_rating_count, PM_total_rating, PM_average_rating, Planner_count, Planner_rating_count, Planner_total_rating, Planner_average_rating, Marketer_count, Marketer_rating_count, Marketer_total_rating, Marketer_average_rating];
...
// 평점 수정
app.put('/admin/grades/edit/:user_id', adminAuth, (req, res) => {
  const user_id = req.params.user_id;
  const updateFields = req.body;
  const allowedFields = [
    ...
    'Designer_count', 'Designer_rating_count', 'Designer_total_rating', 'Designer_average_rating',
    'IOS_count', 'IOS_rating_count', 'IOS_total_rating', 'IOS_average_rating',
    'Android_count', 'Android_rating_count', 'Android_total_rating', 'Android_average_rating',
    'DevOps_count', 'DevOps_rating_count', 'DevOps_total_rating', 'DevOps_average_rating',
    'PM_count', 'PM_rating_count', 'PM_total_rating', 'PM_average_rating',
    'Planner_count', 'Planner_rating_count', 'Planner_total_rating', 'Planner_average_rating',
    'Marketer_count', 'Marketer_rating_count', 'Marketer_total_rating', 'Marketer_average_rating'
  ];
  ...
  if (params.length === 1) {
      console.log('No fields to update', updateFields);
      return res.json({ success: false, error: 'No fields to update' });
  }
...
// 평점 삭제
app.delete('/admin/grades/delete/:user_id', adminAuth, (req, res) => {
  const user_id = req.params.user_id;
  const sql = 'DELETE FROM grade WHERE user_id = ?';

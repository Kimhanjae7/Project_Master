    ...
    connection.query(memberSql, [user_id], (err, memberResult) => {
      ...
      if (memberResult.length === 0) {
        ...
        connection.query(gradeSql, [user_id], (err, gradeResult) => {
          ...
          res.render('myProfile', { member: memberResult[0], grade: gradeData });
        });
      }
    });
  }
  ...
  connection.query(checkSql, [nickname, user_id], function (err, result) {
    ...
    if (result.length > 0) {
      ...
      connection.query(updateSql, values, function (err, result) {
        ...
        res.send("<script> alert('마이페이지가 수정되었습니다.'); location.href='/myProfile';</script>");
      });
    }
  });
});
...
//   if (!link) {
//     link = req.session.member.link;
//     console.log(link);
//   }

//   if (!contact_way) {
//     contact_way = req.session.member.contact_way;
...
//       user_id = ?
//   `;
//   var values = [nickname, link,affiliation, job,  career,interest_stack, introduce, contact_way, user_id];

...
app.get('/grade', (req, res) => {
  ...
  connection.query(sql, [userId], (err, results) => {
    ...
    const projectSeq = results[0].project_seq;
...
app.post('/grade', (req, res) => {
  ...
  const gradeProcedures = userIds.map((uid, i) => {
    if (uid === userId) return Promise.resolve(); // Skip grading self
    ...
    if (results.length > 0 && results[0].check_writer === 'Writer') {
      ...
      connection.query(notifySql, [projectSeq, userId], (err, notifyResults) => {
        ...
        const notifications = notifyResults.map(row => new Promise((resolve, reject) => {
          const insertAlarmSql = `INSERT INTO alarms (user_id, message) VALUES (?, ?)`;
          const message = `프로젝트 "${introduceTitle}" 종료되었습니다. 평점을 남겨주세요.`;
          connection.query(insertAlarmSql, [row.user_id, message], (err) => {
            if (err) return reject(err);
            resolve();
          });
        }));
...
app.get('/alarms', (req, res) => {
  if (req.session.member == null) {
    ...
    const sql = `
      SELECT a.message, p.project_seq, p.introduce_title
      FROM alarms a
      JOIN project_info p ON a.message LIKE CONCAT('%', p.introduce_title, '%')
      WHERE a.user_id = ?
    `;

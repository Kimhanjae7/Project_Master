...
app.get('/profile', (req, res) => { //수정한 부분
...
app.get('/profile_re', (req, res) => { //수정한 부분
  if(req.session.member == null){
    res.send("<script> alert('로그인하고 접근해주세요'); location.href='/';</script>")
  } else {
  res.render('profile_re')
  }
})
...
connection.query(sql, (err, results) => {
  if (err) throw err;
  res.render('board', { projects: results });
});
})
...
app.post('/registrationProc', (req, res) => {
  ...
  connection.query(insertSql, values, function (err, result) {
    if (err) throw err
    res.send("<script> alert('게시글이 등록되었습니다'); location.href='/board';</script>")
  })
})
